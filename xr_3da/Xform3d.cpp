...
#ifndef NDEBUG
	static geXForm3d_MaximalAssertionMode = TRUE;
	#define geXForm3d_Assert if (geXForm3d_MaximalAssertionMode) VERIFY

GENESISAPI 	void GENESISCC geXForm3d_SetMaximalAssertionMode( BOOL Enable )
	{
		VERIFY( (Enable == TRUE) || (Enable == FALSE) );
		geXForm3d_MaximalAssertionMode = Enable;
	}
#else
	#define geXForm3d_Assert VERIFY
#endif


GENESISAPI BOOL GENESISCC geXForm3d_IsValid(const geXForm3d *M)
	...
	// 'valid' means that M is non NULL, and there are no NAN's in the matrix.
{
	...
	if (M == NULL)
		return FALSE;
	if (geVec3d_IsValid(&(M->Translation)) == FALSE)
		return FALSE;
	...
	// sets M to an identity matrix (clears it)
{
	...
	geXForm3d_Assert ( geXForm3d_IsOrthonormal(M) == TRUE );
}

GENESISAPI void GENESISCC geXForm3d_SetXRotation(geXForm3d *M,float RadianAngle)
	// sets up a transform that rotates RadianAngle about X axis
{
	float Cos,Sin;
	VERIFY( M != NULL );
	VERIFY( RadianAngle * RadianAngle >= 0.0f );
...
GENESISAPI void GENESISCC geXForm3d_SetYRotation(geXForm3d *M,float RadianAngle)
...
GENESISAPI void GENESISCC geXForm3d_SetZRotation(geXForm3d *M,float RadianAngle)
...
GENESISAPI void GENESISCC geXForm3d_SetTranslation(geXForm3d *M,float x, float y, float z)
	// sets up a transform that translates x,y,z
{
	...
	M->Translation.Y = y;
	M->Translation.Z = z;
	VERIFY( geVec3d_IsValid(&M->Translation)!=FALSE);
...
GENESISAPI void GENESISCC geXForm3d_SetScaling(geXForm3d *M,float x, float y, float z)
	// sets up a transform that scales by x,y,z
{
	...
	VERIFY( y * y >= 0.0f);
	VERIFY( z * z >= 0.0f);
	VERIFY( x > GEXFORM3D_MINIMUM_SCALE );
	VERIFY( y > GEXFORM3D_MINIMUM_SCALE );
	VERIFY( z > GEXFORM3D_MINIMUM_SCALE );
	...
	geXForm3d_Assert ( geXForm3d_IsOrthogonal(M) == TRUE );
}

GENESISAPI void GENESISCC geXForm3d_RotateX(geXForm3d *M,float RadianAngle)
...
GENESISAPI void GENESISCC geXForm3d_RotateY(geXForm3d *M,float RadianAngle)
...
GENESISAPI void GENESISCC geXForm3d_RotateZ(geXForm3d *M,float RadianAngle)
...
GENESISAPI void GENESISCC geXForm3d_Translate(geXForm3d *M,float x, float y, float z)
...
GENESISAPI void GENESISCC geXForm3d_Scale(geXForm3d *M,float x, float y, float z)
	// Scales M by x,y,z
{
	geXForm3d S;
	VERIFY( M != NULL );
	...
	// MProduct = matrix multiply of M1*M2
{
	...
	VERIFY( M1       != NULL );
	VERIFY( M2       != NULL );
	VERIFY( MProduct != NULL );
	geXForm3d_Assert ( geXForm3d_IsOrthogonal(M1) == TRUE );
	geXForm3d_Assert ( geXForm3d_IsOrthogonal(M2) == TRUE );
	...
	geXForm3d_Assert ( geXForm3d_IsOrthogonal(MProduct) == TRUE );
}
	...
	// Result is Matrix M * Vector V:  V Tranformed by M
{
	...
	geXForm3d_Assert( geVec3d_IsValid(Result)!=FALSE);
...
GENESISAPI void GENESISCC geXForm3d_TransformArray(const geXForm3d *XForm, const geVec3d *Source, geVec3d *Dest, int32 Count)
{
	...
	VERIFY( Source != NULL );
	VERIFY( Dest != NULL );
	geXForm3d_Assert ( geXForm3d_IsOrthogonal(XForm) == TRUE );
	...
	// Gets a vector that is 'left' in the frame of reference of M (facing -Z)
{
	...
	Left->Y = -M->BX;
	Left->Z = -M->CX;
	geXForm3d_Assert( geVec3d_IsValid(Left)!=FALSE);
}
	...
	// Gets a vector that is 'up' in the frame of reference of M (facing -Z)
{
	...
	Up->Y = M->BY;
	Up->Z = M->CY;
	geXForm3d_Assert( geVec3d_IsValid(Up)!=FALSE);
}
	...
	// Gets a vector that is 'in' in the frame of reference of M (facing -Z)
{
	...
	In->Y = -M->BZ;
	In->Z = -M->CZ;
	geXForm3d_Assert( geVec3d_IsValid(In)!=FALSE);
}
...
GENESISAPI void GENESISCC geXForm3d_GetTranspose(const geXForm3d *M, geXForm3d *MInv)
{
	...
	geXForm3d_Assert ( geXForm3d_IsOrthogonal(MInv) == TRUE );
}
	...
	geXForm3d *Dst)
{
	VERIFY( Src != NULL );
	VERIFY( Dst != NULL );
	geXForm3d_Assert ( geXForm3d_IsOrthogonal(Src) == TRUE );
	...
	// order of angles z,y,x
{
	...
	VERIFY( geVec3d_IsValid(Angles)!=FALSE);
}
...
GENESISAPI BOOL GENESISCC geXForm3d_IsOrthonormal(const geXForm3d *M)
...
#define ORTHONORMAL_TOLERANCE ((float)(0.001f))
	...
	geXForm3d_Assert ( geXForm3d_IsValid(M) == TRUE );
...
GENESISAPI BOOL GENESISCC geXForm3d_IsOrthogonal(const geXForm3d *M)
	...
	geXForm3d	*Dest)
{
	...
	geXForm3d_Assert ( geXForm3d_IsOrthogonal(Source) == TRUE );
	VERIFY( PlaneNormal != NULL );
	VERIFY( Dest        != NULL );
	...
	geXForm3d_Assert ( geXForm3d_IsOrthogonal(Dest) == TRUE );
}
