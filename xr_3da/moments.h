...
__forceinline void mean_from_moment(float M[3], moment &m)
{
  M[0] = m.m[0];
  M[1] = m.m[1];
  M[2] = m.m[2];
...
__forceinline void mean_from_accum(float M[3], accum &a)
{
  M[0] = a.m[0] / a.A;
  M[1] = a.m[1] / a.A;
  M[2] = a.m[2] / a.A;
...
__forceinline void covariance_from_accum(float C[3][3], accum &a)
{
  int i,j;
  for(i=0; i<3; i++)
    for(j=0; j<3; j++)
...
__forceinline void compute_moment(moment &M, float p[3], float q[3], float r[3])
{
  float u[3], v[3], w[3];
