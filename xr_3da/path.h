...
GENESISAPI BOOL GENESISCC gePath_GetTimeExtents(
	const gePath *P,
	float *StartTime,
	float *EndTime);
...
GENESISAPI void GENESISCC gePath_GetKeyframe(
	...
	float *Time,			// returns the time of the keyframe
...
GENESISAPI void GENESISCC gePath_Sample(const gePath *P, float Time,geXForm3d *Matrix);
...
GENESISAPI BOOL GENESISCC gePath_OffsetTimes(gePath *P,
	int StartingIndex, int ChannelMask, float TimeOffset );
