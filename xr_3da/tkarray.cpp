...
{
	VERIFY( (A) != NULL );
	VERIFY( ((A)->NumElements == 0) ||
			(((A)->NumElements > 0) && ((A)->Elements != NULL)) );
	VERIFY( (A)->NumElements >= 0 );
	VERIFY( (A)->NumElements <= TK_MAX_ARRAY_LENGTH );
	VERIFY( (A)->ElementSize > 0 );
}
	...
	// is assumed to be the geTKArray_TimeType key.
{
	...
	VERIFY( ElementSize >= sizeof(geTKArray_TimeType) );

	A = (geTKArray *)malloc(TK_ARRAYSIZE);
	if ( A == NULL)
	{
...
geTKArray* GENESISCC geTKArray_CreateFromStream(
	...
	// Creates a new array from the given stream.
{
	...
	pStream->Read(&Header, sizeof(geTKArray_BinaryFileHeader));
//	if (geVFile_Read(pFile, &Header, sizeof(geTKArray_BinaryFileHeader)) == FALSE)
//	{
...
//		geErrorLog_AddString(-1,"Failure to allocate TKArray during binary read", NULL);
	...
	pStream->Read(A->Elements, size - sizeof(geTKArray_BinaryFileHeader));
//	if(geVFile_Read(pFile, A->Elements, size - sizeof(geTKArray_BinaryFileHeader)) == FALSE)
//		{
//			_FREE(A);
...
BOOL GENESISCC geTKArray_SamplesAreTimeLinear(const geTKArray *Array,float Tolerance)
{
	int i;
...
BOOL GENESISCC geTKArray_Insert(
...
BOOL GENESISCC geTKArray_DeleteElement(
