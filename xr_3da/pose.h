...
void GENESISCC gePose_SetScale(gePose *P, const geVec3d *Scale);
	...
	// Returns the index of a joint named JointName.  Returns TRUE if it is
	// located, and Index is set.  Returns FALSE if not, and Index is not changed.
...
void GENESISCC gePose_SetMotion(gePose *P, const geMotion *M,float Time,const geXForm3d *Transform);
...
void GENESISCC gePose_BlendMotion(gePose *P, const geMotion *M, float Time,
...
void GENESISCC gePose_ClearCoverage(gePose *P, int ClearTo);
	...
	// if QueryOnly is FALSE, affected joints are tagged as 'covered', otherwise no changes
