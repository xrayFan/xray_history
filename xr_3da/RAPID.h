...
#define	_RAPID_RAY_PICK_RANGE 1000

int RAPID_RayPick(float R1[3][3], float T1[3], float s1, RAPID_model *RAPID_model1, 
				  float C[3], float D[3], float range = _RAPID_RAY_PICK_RANGE, int flag = RAPID_ALL_CONTACTS);
...
// this is for the client
struct ENGINE_API raypick_info
{
  int id1;
  float range;
};
...
extern  vector<raypick_info> RAPID_raypick;
