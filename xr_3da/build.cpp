...
int RAPID_model::AddTri(const float *p1, const float *p2, const float *p3)
{
	...
	Fvector t1,t2,N;
	Fvector*	v0 = (Fvector*)&p1;
	Fvector*	v1 = (Fvector*)&p2;
	Fvector*	v2 = (Fvector*)&p3;
	t1.sub(*v1,*v0);
	t2.sub(*v2,*v1);
	N.crossproduct(t1,t2);
	N.normalize_safe();
	tris[num_tris].n[0] = N.x;
	tris[num_tris].n[1] = N.y;
	tris[num_tris].n[2] = N.z;
...
int ray::set_ray(float start[3], float dir[3], float _range)
{
	// For a single triangle, orientation is easily determined.
	// The major axis is parallel to the longest edge.
	// The minor axis is normal to the triangle.
