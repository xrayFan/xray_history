...
int XRCollide::add_raypick(raypick_info& rp_inf)
{
    ...
    if (min_raypick_range>rp_inf.range) min_raypick_range = rp_inf.range;
	return RAPID_OK;
}
...
int XRCollide::ray_contact(box *b1, ray *rp)
{
	...
	for(int i=0; i<b1->num_tris; i++){
        ...
        rp_inf.range = 0;
		i1.sMxVpV(XR_ms, XR_mR, model1->tris[rp_inf.id].p1, XR_mT);
		i2.sMxVpV(XR_ms, XR_mR, model1->tris[rp_inf.id].p2, XR_mT);
		i3.sMxVpV(XR_ms, XR_mR, model1->tris[rp_inf.id].p3, XR_mT);
		num_tri_tests++;
//		if (PickFace(&range,c,d,N,i1,i2,i3)) add_raypick(in1, range);
		if (PickFace2(rp->C,rp->D,i1,i2,i3,&rp_inf.range,&rp_inf.u,&rp_inf.v,true)){
            add_raypick(rp_inf);
        }
	}
	return RAPID_OK;
