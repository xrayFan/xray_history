...
//-----------------------------------------------------------------------------
extern "C" void DrawSurface(int x, int y, LPDIRECTDRAWSURFACE7 &pS, const char *name)
{
	HRESULT	hr;
	DDSURFACEDESC2 ddsd;
	ddsd.dwSize=sizeof(ddsd);
	pS->GetSurfaceDesc(&ddsd);
	RECT r;
	r.left=r.top=0;
	r.right=ddsd.dwWidth;
	r.bottom=ddsd.dwHeight;
	if (x < 0) r.left=DWORD(0-x);
	if (y < 0) r.top =DWORD(0-y);
	if (x+r.right >= Device.dwWidth)	r.right -= (x+r.right)-Device.dwWidth;
	if (y+r.bottom >= Device.dwHeight)	r.bottom -= (y+r.bottom)-Device.dwHeight;
	while (true) {
		hr = HW.pBB->BltFast(x,y,pS,&r,DDBLTFAST_SRCCOLORKEY|DDBLTFAST_WAIT);
...
extern "C" LPDIRECTDRAWSURFACE7 LoadSurface(const char *name)
{
	LPDIRECTDRAWSURFACE7 pS;

	pS = DDLoadBitmap(HW.pDDraw,name,0,0);
	VERIFY(pS);
	DDSetColorKey(pS, 0xFF00FF);
	return pS;
}
