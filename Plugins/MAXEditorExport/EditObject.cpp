...
void CEditObject::SaveObject(char* fname){
    CFS_Memory F;
...
void CEditObject::AddMesh(CEditMesh* mesh, const char* name){
	strcpy(mesh->GetName(),name);
	m_Meshes.push_back(mesh);
}

st_Surface*	CEditObject::CreateSurface(Mtl* M, DWORD m_id){
	for (SurfaceIt s_it=m_Surfaces.begin(); s_it!=m_Surfaces.end(); s_it++)
		if (((*s_it)->mat_id==m_id)&&((*s_it)->pMtlMain==M)) return *s_it;
	st_Surface* S	= new st_Surface();
	S->mat_id		= m_id;
	S->pMtlMain		= M;
...
void CEditObject::GenerateSurfaceName(char* name, const char* base_name){
	char nm[128];
	strcpy(nm, base_name);
